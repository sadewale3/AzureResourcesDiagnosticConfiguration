...
foreach ($storageItem in $saList){
    ...
    $resourceID = "/subscriptions/"+$subscription+"/resourceGroups/"+$resourceGroup+"/providers/Microsoft.Storage/storageAccounts/"+$storageAccount+"/queueServices/default"
    $DiagSettingName = $storageAccount+"-queueservice-diag-01"
    ...
    $Output = "Queue Service Diagnostic Setting has been enabled for "+$storageAccount+" Storage Account"
    $Output
}
