...
foreach ($storageItem in $saList){
    ...
    $resourceID = "/subscriptions/"+$subscription+"/resourceGroups/"+$resourceGroup+"/providers/Microsoft.Storage/storageAccounts/"+$storageAccount+"/fileServices/default"
    $DiagSettingName = $storageAccount+"-fileservice-diag-01"
    ...
    $Output = "File Service Diagnostic Setting has been enabled for "+$storageAccount+" Storage Account"
    $Output
}
