...
foreach ($storageItem in $saList){
    ...
    $resourceID = "/subscriptions/"+$subscription+"/resourceGroups/"+$resourceGroup+"/providers/Microsoft.Storage/storageAccounts/"+$storageAccount+"/blobServices/default"
    $DiagSettingName = $storageAccount+"-blobservice-diag-01"
    ...
    $Output = "Blob Service Diagnostic Setting has been enabled for "+$storageAccount+" Storage Account"
    $Output
}
